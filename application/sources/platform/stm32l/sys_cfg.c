...
void sys_ctrl_shell_sw_to_block() {
	while (ring_buffer_char_shell_sending_flag == 1 \
		   && (! ring_buffer_char_is_empty(&ring_buffer_char_shell_send)));

	ENTRY_CRITICAL();

	xprintf_stream_io_out = sys_ctrl_shell_put_char_block;
...
void sys_ctrl_shell_sw_to_nonblock() {
	ENTRY_CRITICAL();
	xprintf_stream_io_out = sys_ctrl_shell_put_char;
	USART_ITConfig(USARTx, USART_IT_TXE, ENABLE);
	EXIT_CRITICAL();
}
