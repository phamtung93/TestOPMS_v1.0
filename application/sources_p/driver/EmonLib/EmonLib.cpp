...
void EnergyMonitor::calcVI(unsigned int crossings, unsigned int timeout) {
	...
	{
		startV = 0;//analogRead(inPinV);                    //using the voltage waveform
	...
	while ((crossCount < crossings) && ((millis()-start)<timeout))
	{
		...
		//-----------------------------------------------------------------------------
		sampleV = 0;//analogRead(inPinV);                 //Read in raw voltage signal
		sampleI = 0;//analogRead(inPinI);                 //Read in raw current signal
...
double EnergyMonitor::calcIrms(unsigned int Number_of_Samples)
{
	...
	if (Number_of_Samples > calcIrms_MAX_NUMBER_SAMPLES) {
		return 0;
	}


//	sampleI = adc_ct_io_read(inPinI);
//	xprintf("Pin:%d :RAW ADC VALUE:%d\n",inPinI,sampleI);

	for (unsigned int n = 0; n < Number_of_Samples; n++) {
		sampleI = adc_ct_io_read(inPinI);//analogRead(inPinI);
